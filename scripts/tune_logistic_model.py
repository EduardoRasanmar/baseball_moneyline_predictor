"""
Hyperparameter tuning for logistic regression model predicting home_win.
...
# Hyperparameter search space (l1_ratio: 0=L2, 1=L1, 0.5=elasticnet)
PARAM_DISTRIBUTIONS = {
...
def main():
    ...
    search = RandomizedSearchCV(
        ...
        scoring="roc_auc",
        random_state=42,
        n_jobs=1,  # use n_jobs=-1 for faster parallel search if your env supports it
        verbose=1,
    )
    ...
    joblib.dump(best_model, MODELS_DIR / "logistic_regression_tuned.pkl")
    joblib.dump(
        {"scaler": best_model.named_steps["scaler"], "feature_names": feat_cols},
        MODELS_DIR / "logistic_model_tuned_meta.pkl",
    )
    ...
    coef.to_csv(TABLES_DIR / "logistic_regression_tuned_coefficients.csv", index=False)
    print(f"Saved coefficients to {TABLES_DIR / 'logistic_regression_tuned_coefficients.csv'}")

    # Save search results
    results_df = pd.DataFrame(search.cv_results_)[
        ["params", "mean_test_score", "std_test_score", "rank_test_score"]
    ].sort_values("rank_test_score")
    results_df.to_csv(TABLES_DIR / "logistic_tuning_cv_results.csv", index=False)
    print(f"Saved CV results to {TABLES_DIR / 'logistic_tuning_cv_results.csv'}")
